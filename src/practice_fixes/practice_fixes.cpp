...
struct PracticeCheckpointData {
    ...
    void applyMacroState() const {
        ...
        if (bot.state == state::playing) {
            ...
            bot.frameOffset = 0;
            bot.updater.frameCount = liveFrame;
            bot.previousFrame = -1;
            bot.updater.overflow = 0.0;
            bot.respawnFrame = -1;

            bot.currentAction = 0;
            while (bot.currentAction < bot.replay.inputs.size() &&
                   bot.replay.inputs[bot.currentAction].frame < liveFrame)
                bot.currentAction++;

            bot.currentFrameFix = 0;
            while (bot.currentFrameFix < bot.replay.frameFixes.size() &&
                   bot.replay.frameFixes[bot.currentFrameFix].frame < liveFrame)
                bot.currentFrameFix++;
